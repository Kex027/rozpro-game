...
int main() {
    ...
    for (int i = 0; i < 20; ++i) {
        Wave w;
        w.pos = Vector2{ static_cast<float>(std::rand() % 1280), static_cast<float>(std::rand() % 720) };
        w.radius = static_cast<float>(10 + std::rand() % 30);
        w.speed = 0.5f + static_cast<float>(std::rand() % 100) / 100.0f;
        w.alpha = static_cast<float>(50 + std::rand() % 150) / 255.0f;
        waves.push_back(w);
    }
            ...
            if (g_shop_open && state.state == 1) {
                ...
                if (found_me) {
                    ...
                    std::vector<ShopItem> shop_items = {
                        ...
                        { 1, "Gold Pan Multiplier", "Złoto is worth 1.5x (Passive)", static_cast<int>(COST_GOLD_MULTIPLIER), my_player.has_gold_multiplier },
                        { 2, "Spiked Base Security", "Slows base thieves to x0.85 (Base Upgrade)", static_cast<int>(COST_BASE_DEFENSE), my_player.has_base_defense },
                        { 3, "Cutlass Sword", "Allows attacking others to steal gold (Combat - F key)", static_cast<int>(COST_ATTACK_WEAPON), my_player.has_attack_weapon },
                        { 4, "Master Thief Mask", "Steal 10% from bases instead of 5% (Passive)", static_cast<int>(COST_THIEF_UPGRADE), my_player.has_thief_upgrade }
                    };
