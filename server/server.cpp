...
void spawn_gold() {
    ...
    for (uint32_t i = 0; i < MAX_GOLD_ITEMS; ++i) {
        if (g_game_state.gold_items[i].is_active) {
            active_count++;
        }
    }
    ...
    for (uint32_t i = 0; i < MAX_GOLD_ITEMS; ++i) {
        if (!g_game_state.gold_items[i].is_active) {
            g_game_state.gold_items[i].id = i;
            ...
            g_game_state.gold_items[i].pos.x = CENTER_X + dist * std::cos(angle);
            g_game_state.gold_items[i].pos.y = CENTER_Y + dist * std::sin(angle);
            
            // Keep gold within screen boundaries
            if (g_game_state.gold_items[i].pos.x < GOLD_RADIUS + 30.0f) g_game_state.gold_items[i].pos.x = GOLD_RADIUS + 30.0f;
            if (g_game_state.gold_items[i].pos.x > MAP_WIDTH - GOLD_RADIUS - 30.0f) g_game_state.gold_items[i].pos.x = MAP_WIDTH - GOLD_RADIUS - 30.0f;
            if (g_game_state.gold_items[i].pos.y < GOLD_RADIUS + 30.0f) g_game_state.gold_items[i].pos.y = GOLD_RADIUS + 30.0f;
            if (g_game_state.gold_items[i].pos.y > MAP_HEIGHT - GOLD_RADIUS - 30.0f) g_game_state.gold_items[i].pos.y = MAP_HEIGHT - GOLD_RADIUS - 30.0f;
            
            // Random gold value between 5 and 15
            g_game_state.gold_items[i].value = 5 + (std::rand() % 11);
            g_game_state.gold_items[i].is_active = true;
            
            // Alert players of gold spawn
            MsgServerAlert alert;
            std::sprintf(alert.message, "A new gold nugget was deposited near (%.0f, %.0f)", g_game_state.gold_items[i].pos.x, g_game_state.gold_items[i].pos.y);
...
void reset_round() {
    g_game_state.round_timer = static_cast<float>(ROUND_DURATION);
    g_game_state.winner_id = 0;
    
    // Clear gold items
    for (uint32_t i = 0; i < MAX_GOLD_ITEMS; ++i) {
        g_game_state.gold_items[i].is_active = false;
    }
    ...
    for (uint32_t i = 0; i < g_game_state.player_count; ++i) {
        Base& b = g_game_state.bases[i];
        b.owner_id = g_game_state.players[i].id;
        b.angle = (2.0f * 3.14159f / MAX_PLAYERS) * g_game_state.players[i].color_index;
        b.pos.x = CENTER_X + BASE_ROTATION_RADIUS * std::cos(b.angle);
        b.pos.y = CENTER_Y + BASE_ROTATION_RADIUS * std::sin(b.angle);
        b.is_active = g_game_state.players[i].is_active;
    }
...
void client_handler(SOCKET client_socket, uint32_t player_id) {
    ...
    while (g_server_running) {
        ...
        int player_idx = -1;
        for (uint32_t i = 0; i < g_game_state.player_count; ++i) {
            if (g_game_state.players[i].id == player_id) {
                player_idx = i;
                break;
            }
        }
        ...
        Player& player = g_game_state.players[player_idx];
        ...
        switch (header.type) {
            case MSG_CLIENT_READY: {
                if (g_game_state.state == 0) { // Only in Lobby
            ...
            case MSG_CLIENT_BUY: {
                ...
                if (player.gold_in_base >= cost) {
                    bool bought = false;
                    if (item == 0 && !player.has_speed_upgrade) {
                        player.has_speed_upgrade = true;
                        bought = true;
    ...
    for (uint32_t i = 0; i < g_game_state.player_count; ++i) {
        if (g_game_state.players[i].id == player_id) {
            g_game_state.players[i].is_active = false;
            g_game_state.players[i].dir.x = 0;
            g_game_state.players[i].dir.y = 0;
            
            // Mark their base inactive
            for (uint32_t j = 0; j < g_game_state.player_count; ++j) {
                if (g_game_state.bases[j].owner_id == player_id) {
                    g_game_state.bases[j].is_active = false;
                }
            }
            
            MsgServerAlert alert;
            std::sprintf(alert.message, "%s left the game.", g_game_state.players[i].name);
            broadcast_packet(MSG_SERVER_ALERT, &alert, sizeof(alert));
            break;
        }
    }
...
void game_tick_loop() {
    ...
    while (g_server_running) {
        ...
        
        {
            ...
            // Game State Machine
            if (g_game_state.state == 0) { // Lobby
                // Check if everyone connected is ready (requires at least 1 player for testing/debugging)
                bool all_ready = (g_game_state.player_count > 0);
                for (uint32_t i = 0; i < g_game_state.player_count; ++i) {
                    if (g_game_state.players[i].is_active && !g_game_state.players[i].is_ready) {
                        all_ready = false;
                    }
                }
                
                if (all_ready) {
                    g_game_state.state = 1; // Start playing
                    g_game_state.round_number = 1;
                    reset_round();
            ...
            } else if (g_game_state.state == 1) { // Playing
                // Decrement timers
                g_game_state.round_timer -= dt;
                ...
                for (uint32_t i = 0; i < g_game_state.player_count; ++i) {
                    ...
                    for (uint32_t j = 0; j < MAX_GOLD_ITEMS; ++j) {
                        GoldItem& gold = g_game_state.gold_items[j];
                ...
                for (uint32_t i = 0; i < g_game_state.player_count; ++i) {
                    ...
                    for (uint32_t j = i + 1; j < g_game_state.player_count; ++j) {
                ...
                if (g_game_state.round_timer <= 0) {
                    ...
                    if (winner_idx != -1 && !tie) {
                        g_game_state.winner_id = g_game_state.players[winner_idx].id;
                        g_game_state.players[winner_idx].rounds_won++;
                        
                        MsgServerAlert alert;
                        std::sprintf(alert.message, "%s wins Round %d with %d gold!", 
                                     g_game_state.players[winner_idx].name, 
                                     g_game_state.round_number, max_gold);
                        broadcast_packet(MSG_SERVER_ALERT, &alert, sizeof(alert));
                    } else {
                        g_game_state.winner_id = 0; // Tie or no gold
                        MsgServerAlert alert;
                        std::sprintf(alert.message, "Round %d ends in a tie!", g_game_state.round_number);
                        broadcast_packet(MSG_SERVER_ALERT, &alert, sizeof(alert));
                    }
                    
                    // Check if game is completely over
                    if (g_game_state.round_number >= TOTAL_ROUNDS) {
                        g_game_state.state = 3; // Game Over
                        ...
                        if (match_winner_idx != -1 && !match_tie) {
                            g_game_state.winner_id = g_game_state.players[match_winner_idx].id;
                        } else {
                            g_game_state.winner_id = 0; // Joint 1st place / Tie
                        }
                    } else {
                        g_game_state.state = 2; // Round End transition
                        state_timer = 5.0f;    // Show round scores screen for 5s
                    }
                }
                
            } else if (g_game_state.state == 2) { // Round End screen
                state_timer -= dt;
                if (state_timer <= 0) {
                    g_game_state.round_number++;
                    g_game_state.state = 1; // Resume playing next round
                    reset_round();
                    
                    MsgServerAlert alert;
                    std::sprintf(alert.message, "Round %d begins!", g_game_state.round_number);
                    broadcast_packet(MSG_SERVER_ALERT, &alert, sizeof(alert));
                }
            } else if (g_game_state.state == 3) { // Game Over screen
                state_timer -= dt;
                if (state_timer <= 0) {
                    // Reset everything to Lobby
                    g_game_state.state = 0;
                    g_game_state.round_number = 0;
                    g_game_state.player_count = 0;
                    g_game_state.gold_count = 0;
                    g_clients.clear();
            ...
            broadcast_packet(MSG_SERVER_STATE, &g_game_state, sizeof(g_game_state));
        }
...
int main() {
    ...
    if (listenfd == INVALID_SOCKET) {
        std::cerr << "[Server] Socket creation failed: " << get_last_socket_error() << std::endl;
        return 1;
    }
    ...
    if (bind(listenfd, reinterpret_cast<struct sockaddr*>(&serv_addr), sizeof(serv_addr)) == SOCKET_ERROR) {
        std::cerr << "[Server] Bind failed: " << get_last_socket_error() << std::endl;
        close_socket(listenfd);
        return 1;
    }

    if (listen(listenfd, 10) == SOCKET_ERROR) {
        std::cerr << "[Server] Listen failed: " << get_last_socket_error() << std::endl;
        close_socket(listenfd);
        return 1;
    }
    ...
    for (;;) {
        ...
        if (connfd == INVALID_SOCKET) {
            std::cerr << "[Server] Accept failed: " << get_last_socket_error() << std::endl;
            continue;
        }
        ...
        if (g_game_state.state != 0 || g_game_state.player_count >= MAX_PLAYERS) {
        ...
        uint32_t color_idx = g_game_state.player_count;
        ...
        Player& new_player = g_game_state.players[g_game_state.player_count];
